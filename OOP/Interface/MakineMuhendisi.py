...
class MakineMuhendisi(implements(IMuhendis.IMuhendis,ICalisma.ICalisma)):
    ...
    def is_tecrubesi(self, array=[]):
        if len(array)>0:
            print("Makine Mühendisi Olarak Şu Şirketlerde Çalıştım...")
            for i in array:
                print(i, end=" ")
        else:
    ...
    def calis(self):
        print("Makine Mühendisi Çalışıyor")

    def referans_getir(self,array=[]):
        if len(array) > 0:
            print("Referanslar:")
            for i in array:
                print(i, end=" ")
        else:
            print("Herhangi bir referansım bulunmuyor...")
